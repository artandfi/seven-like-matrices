'''
    A determinant of a VM-7 matrix, that is, a matrix of form like this:
...
def det_vm7_structure_preserving(matrix):
    # Calculate cumulative prods of a_i
    prods_a = [matrix[n - i - 1][n - i - 1] for i in range(1, n)]
...
def det_vm7_structure_modifying(matrix):
    # Calculate cumulative prods of a_i
...
det_sp = det_vm7_structure_preserving(matrix)
det_sm = det_vm7_structure_modifying(matrix)



print(f"VM-7 regular det: {det_numpy}")
print(f"VM-7 structure-preserving det: {det_sp}")
print(f"VM-7 structure-modifying det : {det_sm}")
